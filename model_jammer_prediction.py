...
class JammerPositionPredictor:
    ...
    def predict(self, input_sequence, terrain_patch=None, num_samples=100):
        ...
        if self.terrain_model is not None and terrain_patch is not None:
            ...
            # This is the key fix - use the terrain_feature_dim from config instead of input dim
    ...
    def save_model(self, filename):
        ...
        try:
            ...
            # Fallback for older PyTorch versions or if the function doesn't exist
            pass
        ...
        # Use weights_only=False to allow serialization of non-tensor objects
...
if __name__ == "__main__":
    # Make sure StandardScaler is in the safe globals list for PyTorch 2.6+
    try:
        import torch.serialization
        from sklearn.preprocessing import StandardScaler
        # Add StandardScaler to the list of safe globals
        torch.serialization.add_safe_globals([StandardScaler])
        print("Added StandardScaler to PyTorch's safe globals list")
    except (ImportError, AttributeError) as e:
        print(f"Could not add StandardScaler to safe globals: {e}")
        print("This may be fine for PyTorch versions before 2.6")
