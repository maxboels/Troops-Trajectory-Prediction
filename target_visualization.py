...
class TargetVisualizer:
    ...
    def __init__(self, terrain_data=None, elevation_data=None):
        ...
        # Set up a proper topographical colormap
        self.terrain_cmap = self._create_terrain_colormap()
        
        # Create continuous elevation colormap similar to topographical maps
        self.elevation_cmap = self._create_elevation_colormap()
        
        # Detect if we need to normalize the terrain data
        if self.terrain_data is not None:
            self._analyze_terrain_data()
    
    def _analyze_terrain_data(self):
        """Analyze terrain data to understand its structure."""
        self.terrain_min = np.min(self.terrain_data)
        self.terrain_max = np.max(self.terrain_data) 
        self.terrain_unique = np.unique(self.terrain_data)
        
        print(f"Terrain data range: {self.terrain_min} to {self.terrain_max}")
        print(f"Unique terrain values: {self.terrain_unique}")
        
        # Auto-detect if this is a categorical or continuous dataset
        if len(self.terrain_unique) < 30 and np.all(np.floor(self.terrain_unique) == self.terrain_unique):
            self.terrain_categorical = True
            print("Detected categorical terrain data")
        else:
            self.terrain_categorical = False
            print("Detected continuous terrain data (likely elevation)")
    
    def _create_terrain_colormap(self):
        """Create a standardized colormap for terrain visualization."""
        # Create a terrain colormap based on land use categories
        # These should match what's in the LandUseCategory enum
        terrain_colors = [
            '#F5DEB3',  # 0: Land/Agricultural (tan)
            '#228B22',  # 1: Forest (forest green)
            '#90EE90',  # 2: Grassland (light green)
            '#808080',  # 3: Urban (gray)  
            '#0000FF',  # 4: Water bodies (blue)
            '#00FFFF',  # 5: Wetland (cyan)
            '#A0522D',  # 6: Barren/Mountain (brown)
        ]
        
        # Create a ListedColormap
        return plt.cm.colors.ListedColormap(terrain_colors)
    
    def _create_elevation_colormap(self):
        """Create a topographical style colormap for elevation data."""
        # Create a colormap similar to typical topographical maps
        # Blue for water, green for low elevation, yellow to red for higher
        from matplotlib.colors import LinearSegmentedColormap
        
        # Add more control points for a smoother transition
        colors = [
            (0.0, '#0000FF'),   # Deep blue for below sea level
    ...
    def _add_terrain_background(self, ax, lon_min, lon_max, lat_min, lat_max):
        """Add terrain background to the given axes with proper orientation and mapping."""
        if self.elevation_data is not None:
            # Use elevation data for a topographically accurate visualization
            # This produces a visualization similar to the reference map
            
            # Create a normalized colormap for better elevation visualization
            vmin = np.min(self.elevation_data)
            vmax = np.max(self.elevation_data)
            
            # Set water areas to be blue (all values below 0)
            if vmin < 0:
                # Make values below sea level (negative) blue
                norm = plt.cm.colors.TwoSlopeNorm(vmin=vmin, vcenter=0, vmax=vmax)
            else:
                norm = plt.cm.colors.Normalize(vmin=vmin, vmax=vmax)
            
            # Use proper orientation with origin='lower' for correct North-South orientation
            im = ax.imshow(
                self.elevation_data, 
                ...
                aspect='auto', 
                origin='lower',  # Correct orientation parameter
                alpha=0.8, 
                zorder=0
            )
        ...
        elif self.terrain_data is not None:
            # If we have terrain data but not elevation, use that
            
            # Check if we need to process the terrain data for visualization
            if hasattr(self, 'terrain_categorical') and self.terrain_categorical:
                # Display categorical terrain data using appropriate colormap
                display_terrain = self.terrain_data
            else:
                # Convert to land use categories if needed
                # (This is a fallback in case we have unclassified terrain data)
                display_terrain = np.copy(self.terrain_data)
                
                # Map water bodies
                water_mask = (self.terrain_data == 20) | (self.terrain_data == 4)
                display_terrain[water_mask] = 4  # Water bodies (blue)
                
                # Map other categories
                display_terrain[(self.terrain_data >= 1) & (self.terrain_data <= 5)] = 1  # Forest  
                display_terrain[(self.terrain_data >= 6) & (self.terrain_data <= 10)] = 2  # Grassland
                display_terrain[self.terrain_data == 18] = 3  # Urban
                display_terrain[self.terrain_data == 15] = 5  # Wetland
                display_terrain[(self.terrain_data >= 16) & (self.terrain_data <= 17)] = 6  # Barren
            
            # Ensure proper axis limits are set to match the terrain bounds
            ax.imshow(
                display_terrain, 
                cmap=self.terrain_cmap, 
                extent=[lon_min, lon_max, lat_min, lat_max],
                aspect='auto', 
                origin='lower',  # Use 'lower' to match the correct orientation
                alpha=0.8, 
                zorder=0
            )
            ...
            terrain_legend = [
                Patch(facecolor='#F5DEB3', label='Agricultural/Land'),
                ...
                Patch(facecolor='#A0522D', label='Barren/Mountain')
            ]
            ...
            # Add the terrain legend
            ax.add_artist(terrain_legend)
        else:
            # If no terrain or elevation data, add a basic grid
    ...
    def visualize_single_target(self, target_data, target_id, timestamp, prediction_result, 
        ...
        # --------------------------------------------
        # Use the same bounds for both plots for consistency
        # We'll focus on the region around the target rather than showing the entire terrain
        # Expand the bounds a bit more for context
        context_padding = 0.2  # Larger padding for wider context
        context_lon_min = lon_min - lon_range * context_padding
        context_lon_max = lon_max + lon_range * context_padding
        context_lat_min = lat_min - lat_range * context_padding
        context_lat_max = lat_max + lat_range * context_padding
        
        # Add terrain background - use the context bounds
        self._add_terrain_background(ax2, context_lon_min, context_lon_max, context_lat_min, context_lat_max)
        
        # Set axis limits explicitly for the terrain view
        ax2.set_xlim(context_lon_min, context_lon_max)
        ax2.set_ylim(context_lat_min, context_lat_max)
    ...
    def visualize_predictions_on_terrain(self, target_data, predictions_by_target_id, timestamp, 
        ...
        for target_id, prediction in predictions_by_target_id.items():
            ...
            ax.annotate(
                ...
                fontsize=9, color="black",  # Black text for better visibility
        ...
        # Add a scaled colorbar for the terrain legend
        # This is cleaner than the previous legend approach
        terrain_legend = [
            Patch(facecolor='#F5DEB3', label='Agricultural'),
            Patch(facecolor='#90EE90', label='Grassland'),
            Patch(facecolor='#228B22', label='Forest'),
            Patch(facecolor='#808080', label='Urban'),
            Patch(facecolor='#0000FF', label='Water'),
        ...
        # Create separate legend for terrain types
        terrain_legend = ax.legend(
            handles=terrain_legend, 
            loc='lower right',
            title='Terrain Type',
        ...
        # Add the terrain legend separately
        ax.add_artist(terrain_legend)
