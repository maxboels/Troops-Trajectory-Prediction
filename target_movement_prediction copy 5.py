...
def create_prediction_animation(predictor, output_filename='target_prediction_animation.mp4'):
    ...
    # The key fix is using origin='upper' to properly orient the terrain map
    terrain_img = ax.imshow(terrain_map, cmap=terrain_cmap, alpha=0.5, 
                         extent=[lon_min, lon_max, lat_max, lat_min],  # Note the reversed lat order
...
def main():
    ...
    parser.add_argument('--mode', type=str, required=True, choices=['train', 'eval', 'infer', 'visualize', 'test'],
