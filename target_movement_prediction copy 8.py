...
class TargetTrajectoryPredictor(nn.Module):
    ...
    def forward(self, pos_seq, terrain_patch):
        ...
        causal_mask = torch.triu(torch.ones(seq_len, seq_len), diagonal=1).bool().to(pos_seq.device)
...
def predict_trajectory(model, initial_sequence, terrain_data, time_delta=60, steps=30):
    """
    Predict target trajectory for the next 60 minutes.
    
    Args:
        model: Trained model
        initial_sequence: Initial position sequence [seq_len, features]
        terrain_data: Terrain map
        time_delta: Time step in seconds (default: 60s = 1 minute)
        steps: Number of steps to predict (default: 30 steps = 30 minutes)
    ...
    with torch.no_grad():
        for step in range(steps):
            # Extract terrain at current position (would need implementation)
            current_pos = current_sequence[0, -1, :2].cpu().numpy()  # Last position
            terrain_patch = extract_terrain_patch(terrain_data, current_pos)
            terrain_patch = torch.tensor(terrain_patch, dtype=torch.float32).unsqueeze(0).to(device)
            
            # Predict next position
            mean, logvar = model(current_sequence, terrain_patch)
            
            # Take the last prediction
            next_pos_mean = mean[:, -1:, :]
            next_pos_logvar = logvar[:, -1:, :]
            
            # Calculate confidence interval
            std = torch.exp(0.5 * next_pos_logvar)
            lower_ci = next_pos_mean - 1.96 * std
            upper_ci = next_pos_mean + 1.96 * std
            
            # Store predictions
            all_predictions.append(next_pos_mean.cpu().numpy())
            all_uncertainties.append({
                'lower': lower_ci.cpu().numpy(),
                'upper': upper_ci.cpu().numpy()
            })
            ...
            current_sequence = torch.cat([
                ...
                torch.cat([next_pos_mean, current_sequence[:, -1:, 2:]], dim=2)  # Add new position
            ], dim=1)
    
    # Concatenate results
    predictions = np.concatenate(all_predictions, axis=1)
    lower_bounds = np.concatenate([u['lower'] for u in all_uncertainties], axis=1)
    upper_bounds = np.concatenate([u['upper'] for u in all_uncertainties], axis=1)
    
    return {
        'mean': predictions.squeeze(0),
        'lower_ci': lower_bounds.squeeze(0),
        'upper_ci': upper_bounds.squeeze(0)
    }
