...
def create_prediction_animation(predictor, output_filename='target_prediction_animation.mp4'):
    ...
    # Calculate terrain extent to match the coordinate system
    # The key fix is here - correctly mapping terrain indices to lat/lon coordinates
    terrain_height, terrain_width = terrain_map.shape
    
    # Add terrain background with proper extent
    terrain_img = ax.imshow(terrain_map, cmap=terrain_cmap, alpha=0.5, 
                          extent=[lon_min, lon_max, lat_min, lat_max], 
    ...
    def update(frame_idx):
        ...
        for target_id in list(prediction_lines.keys()):
            if prediction_lines[target_id] in ax.lines:
                prediction_lines[target_id].remove()  # Use remove() on the line object directly
        ...
        for target_id in list(target_trails.keys()):
            if target_trails[target_id] in ax.lines:
                target_trails[target_id].remove()  # Use remove() on the line object directly
...
def predict_test_set(predictor, test_data_path, output_path, prediction_duration=300):
    ...
    # Get unique target IDs and timestamps
...
def visualize_target_prediction_with_terrain(
    ...
    """
    Enhanced visualization function that properly displays terrain data.
    """
    ...
    # Get coordinate bounds
    lon_min, lon_max = prediction['mean'][:, 0].min() - 0.05, prediction['mean'][:, 0].max() + 0.05
    lat_min, lat_max = prediction['mean'][:, 1].min() - 0.05, prediction['mean'][:, 1].max() + 0.05
    ...
    ax1.plot(mean_traj[:, 0], mean_traj[:, 1], 'r-', label='Predicted Trajectory')
    ...
    if terrain_data is not None:
        ...
        terrain_cmap = plt.cm.colors.ListedColormap(terrain_colors)
        
        # Calculate appropriate extent for the region of interest
        # This is a key fix - ensuring terrain is properly mapped to coordinates
        terrain_height, terrain_width = terrain_data.shape
        
        # Get the full extent of the target data
        ...
        ax2.plot(mean_traj[:, 0], mean_traj[:, 1], 'r--', linewidth=2)
...
def main():
    """
    Main entry point with improved structure for training, validation, and inference.
    """
    import argparse
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Target Movement Prediction')
    parser.add_argument('--mode', type=str, required=True, choices=['train', 'eval', 'infer', 'visualize'],
                        help='Mode to run: train, eval, infer, or visualize')
    ...
    if args.mode == 'train':
        ...
        if args.target_id:
            ...
            print(f"Prediction visualization saved to {os.path.join(args.output_dir, f'prediction_{target_id}.png')}")
        else:
...
#         'learning_rate': 1e-4,
#         'batch_size': 32,
#         'num_epochs': 100,  # Reduce for faster training during development
#         'sequence_length': 10,
#         'prediction_horizon': 5,
#         'terrain_feature_dim': 32,
#         'use_terrain': True,
...
#     # Train model
#     if not config['train']:
#         # Initialize predictor (no training)
#         predictor = TargetMovementPredictor(
#             config=config,
...
#         model_path = 'models/target_prediction/target_predictor_model.pt'  # Adjust path as needed
...
#         # Find a timestamp with multiple targets
#         # Choose a timestamp where multiple targets exist
#         timestamps = sorted(target_data['datetime'].unique())
#         mid_idx = len(timestamps) // 2  # Take a timestamp from the middle of the dataset
#         prediction_timestamp = timestamps[mid_idx]
...
#             terrain_data=terrain_data,
#             blue_force_data=blue_force_data,
#             output_path='all_targets_prediction_visualization.png',
#             use_different_colors=True
#         )
...
#         print(f"Creating visualization for target {target_id}...")
#         visualize_out_of_view_prediction(
#             predictor,
#             target_data,
#             target_id,
...
#         # Create animation (optional - this may take some time)
